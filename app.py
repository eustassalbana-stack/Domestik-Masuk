import streamlit as st
import pandas as pd

st.set_page_config(page_title="📦 Aplikasi Data Komoditas Ekspor", layout="wide")

# --- Judul Utama ---
st.title("📦 Aplikasi Pencarian Data Komoditas Ekspor")

# --- Upload File Excel ---
uploaded_file = st.file_uploader("Unggah file Excel Anda", type=["xlsx"])

# --- Upload file kabupaten-provinsi ---
st.sidebar.header("📍 Dataset Wilayah")
mapping_file = st.sidebar.file_uploader("Unggah file kabupaten–provinsi (CSV)", type=["csv"])

if uploaded_file:
    df = pd.read_excel(uploaded_file)
    df.columns = df.columns.str.strip().str.lower()

    # Kolom wajib
    required_cols = [
        "daerah asal", "daerah tujuan", "klasifikasi",
        "komoditas", "nama tercetak", "kode hs", "satuan"
    ]

    if not all(col in df.columns for col in required_cols):
        st.error(f"❌ Kolom wajib tidak lengkap. Pastikan file Anda memiliki kolom berikut:\n{', '.join(required_cols)}")

    else:
        # --- Baca data kabupaten–provinsi ---
        if mapping_file is not None:
            map_df = pd.read_csv(mapping_file)
            map_df.columns = map_df.columns.str.lower().str.strip()

            # pastikan ada kolom kabupaten_kota dan provinsi
            if not all(col in map_df.columns for col in ["kabupaten_kota", "provinsi"]):
                st.error("❌ File mapping harus memiliki kolom: kabupaten_kota dan provinsi.")
            else:
                # Standarisasi lowercase
                df["daerah_asal_clean"] = df["daerah asal"].str.lower().str.strip()
                map_df["kabupaten_kota_clean"] = map_df["kabupaten_kota"].str.lower().str.strip()

                # Merge untuk dapatkan provinsi asal
                df = df.merge(
                    map_df[["kabupaten_kota_clean", "provinsi"]],
                    left_on="daerah_asal_clean",
                    right_on="kabupaten_kota_clean",
                    how="left"
                )

                df = df.rename(columns={"provinsi": "provinsi asal"})
                df["provinsi asal"] = df["provinsi asal"].fillna("Provinsi tidak diketahui")

                # --- Sidebar Pilihan Komoditas ---
                st.sidebar.header("🔍 Pilih Komoditas")
                komoditas_list = sorted(df["komoditas"].dropna().unique())
                selected_komoditas = st.sidebar.selectbox("Pilih komoditas:", komoditas_list)

                filtered_df = df[df["komoditas"] == selected_komoditas].copy()

                # --- Status Pemeriksaan Komoditas ---
                if "checked_items" not in st.session_state:
                    st.session_state.checked_items = {}

                checked = st.checkbox(
                    "✅ Tandai komoditas ini sudah diperiksa",
                    value=st.session_state.checked_items.get(selected_komoditas, False)
                )
                st.session_state.checked_items[selected_komoditas] = checked

                # --- Buat kolom gabungan untuk rute ---
                filtered_df["daerah asal/tujuan"] = filtered_df.apply(
                    lambda row: f"{row['daerah asal']} → {row['daerah tujuan']}" 
                    if row['daerah asal'].strip().lower() != row['daerah tujuan'].strip().lower()
                    else row['daerah asal'],
                    axis=1
                )

                # --- Hapus duplikasi berdasarkan rute dan kolom penting lainnya ---
                filtered_df = filtered_df.drop_duplicates(
                    subset=[
                        "daerah asal/tujuan", 
                        "komoditas", 
                        "klasifikasi", 
                        "provinsi asal", 
                        "nama tercetak", 
                        "kode hs", 
                        "satuan"
                    ]
                )

                # --- Tampilkan Data ---
                if not filtered_df.empty:
                    st.subheader(f"📊 Informasi Komoditas: {selected_komoditas}")
                    st.dataframe(
                        filtered_df[
                            [
                                "provinsi asal",
                                "daerah asal/tujuan",
                                "klasifikasi",
                                "komoditas",
                                "nama tercetak",
                                "kode hs",
                                "satuan"
                            ]
                        ],
                        use_container_width=True
                    )

                    if checked:
                        st.success(f"✅ Komoditas **{selected_komoditas}** telah diperiksa.")
                    else:
                        st.info(f"🔎 Komoditas **{selected_komoditas}** belum diperiksa.")
                else:
                    st.warning("Tidak ada data untuk komoditas ini.")

                # --- Unduh hasil ---
                @st.cache_data
                def convert_to_excel(df_export):
                    from io import BytesIO
                    buffer = BytesIO()
                    df_export.to_excel(buffer, index=False, engine='openpyxl')
                    return buffer.getvalue()

                st.download_button(
                    label="💾 Unduh Hasil (Excel)",
                    data=convert_to_excel(filtered_df),
                    file_name=f"hasil_{selected_komoditas}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
        else:
            st.warning("📂 Silakan unggah file kabupaten–provinsi (CSV) terlebih dahulu.")
else:
    st.info("⬆️ Silakan unggah file Excel terlebih dahulu untuk memulai.")

st.markdown("---")
st.caption("Dibuat dengan ❤️ menggunakan Streamlit")
